...
- 滚动锁定：已完成（打开时锁定页面滚动，可嵌套，并保留消费方原有的 `overflow`）
- Portal 层：**有意不做**（"渲染到别处"＝换一个 host，而非新机制，加层会让每个渲染器各自实现搬运）。
- `container` 逃生口：已完成。传入应用自己的容器元素即把 Content / Overlay 移动过去；默认不传则行为不变。
- 有意不做：动画
