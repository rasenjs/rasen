...
export interface ShadowItem {
  ...
  transform: Mat4x4f
  /** Optional triangle index list — indexed BatchItems expand through it so
   * the depth pass draws the same triangles the color pass draws. */
...
export class ShadowRenderer {
  ...
  render(items: ShadowItem[], lightMatrix: Mat4x4f) {
    ...
    // Compute total vertices to size scratch buffers (indexed items expand
    // through their index list — same triangle count the color pass draws)
    const totalVerts = items.reduce((s, it) => s + (it.indices && it.indices.length > 0 ? it.indices.length : it.vertices.length / 3), 0)
    ...
    for (const item of items) {
      ...
      const idx = item.indices && item.indices.length > 0 ? item.indices : null
      const count = idx ? idx.length : v.length / 3
      for (let i = 0; i < count; i++) {
        const s = idx ? idx[i] * 3 : i * 3
        const x = v[s], y = v[s + 1], z = v[s + 2]
