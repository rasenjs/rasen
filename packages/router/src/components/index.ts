...
import { match, getReactiveRuntime, type Mountable } from '@rasenjs/core'
import type { 
  Router, 
  Route, 
  RouteMatch,
...
export function makeRouterReactive<TRoutes extends Record<string, any>>(
  ...
  // 初始化
  runtime.setValue(currentMatchRef, router.current)

  // 监听所有路由变化
  router.afterEach((to) => {
    runtime.setValue(currentMatchRef, to)
  })
  ...
  return {
    ...router,
    get current() {
      return runtime.unref(currentMatchRef)
    }
  }
}
