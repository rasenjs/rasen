 ...
 * })
 */
export function switchCase<Host = unknown, K extends string = string, N = unknown>(
  ...
  return (host: Host) => {
    ...
    let currentKey: K | null | undefined | typeof UNINITIALIZED = UNINITIALIZED
    // 当前分支的卸载函数
    ...
    const mountBranch = (key: K | null | undefined) => {
      // 获取对应的工厂函数
      let factory: ((key: K) => Mountable<Host>) | (() => Mountable<Host>) | undefined
      ...
      // 创建并挂载组件
      const mountable = key != null && config.cases[key] 
