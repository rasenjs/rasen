 ...
 * each 内部配置
 */
interface EachImplConfig<T extends object, N = unknown> {
  items: () => T[]
  render: (item: T, index: number) => Mountable<N>
 ...
 * ```
 */
export function repeat<T, N = unknown>(
  items: Ref<T[]> | (() => T[]),
  render: (item: T, index: number) => Mountable<N>
): Mountable<N>
...
export function repeat<T, N = unknown>(
  ...
  render:
    | ((item: T, index: number) => Mountable<N>)
    | ((index: number) => Mountable<N>)
): Mountable<N> {
  return repeatImpl<T, N>({
    items: () => {
    ...
    render: render as (item: T, index: number) => Mountable<N>
  })
}
 ...
 * repeat 内部配置
 */
interface RepeatImplConfig<T, N = unknown> {
  items: () => T[]
  render: (item: T, index: number) => Mountable<N>
