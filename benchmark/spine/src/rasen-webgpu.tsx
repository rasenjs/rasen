/**
 * Rasen WebGPU entry — @rasenjs/dom canvas + @rasenjs/gfx spine component.
 *
...
// WebGPU device must exist BEFORE the canvas mounts (the bridge mount is
// sync; requestDevice is async). The Stage only renders once it resolves.
const webgpuDevice = shallowRef<unknown>(null)
{
  const nav = navigator as unknown as Record<string, { requestAdapter(): Promise<unknown> }>
  const gpu = nav.gpu
  if (!gpu) throw new Error('WebGPU unavailable: navigator.gpu missing — this bench page requires it')
  gpu.requestAdapter().then(async (a) => {
    if (!a) throw new Error('WebGPU: requestAdapter returned null')
    webgpuDevice.value = await (a as { requestDevice(): Promise<unknown> }).requestDevice()
  })
}
...
const Stage = com(() => (
  <canvas
    ...
    camera={camera}
  >
    {webgpuDevice.value && each(instances, (inst: Instance) => (
      <SpineWebgl
...
// Mount only AFTER the device resolves — the bridge mount is sync and
// requires the webgpuDevice prop to be populated at that moment.
const stage = installStage()
void Promise.resolve().then(async () => {
  // wait for the device shallowRef to be populated (polled; it resolves once)
  while (!webgpuDevice.value) await new Promise((r) => setTimeout(r, 16))
  mount(App(), stage)
})
...
const bench: BenchAPI = {
  async load() {
    return timed(async () => {
      ...
      ;(window as any).__skelProbe = instances.value[0]
      await nextPaint()
    })
  },
