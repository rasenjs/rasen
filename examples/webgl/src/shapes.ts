...
function createExample(
  ...
  height: number,
  content:
    | MountFunction<WebGLRenderingContext | WebGL2RenderingContext>
    | MountFunction<WebGLRenderingContext | WebGL2RenderingContext>[]
) {
  return div({
    class: 'example-card',
...
const pageHeader = div({
  class: 'page-header',
  children: [
    h1({ children: ['🎨 WebGL 2D Shapes'] }),
    p({ children: ['GPU-accelerated 2D shapes with the same API as canvas-2d'] })
  ]
})
