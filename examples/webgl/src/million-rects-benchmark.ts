/**
 * Million Rects Benchmark - WebGL 版本
 *
 * 测试 100 万个可交互矩形的：
 * - 首屏创建速度（使用 rect() 组件）
 * - 内存占用
...
import { rect } from '@rasenjs/webgl'
...
// ============================================================================
// WebGL Rendering - 使用 rect() 组件创建 100 万个矩形
// ============================================================================
...
const pageHeader = div({
  class: 'page-header',
  children: [
    h1({ children: ['🚀 Million Rectangle Benchmark - WebGL'] }),
    p({
      children: [
        'Performance test with 1,000,000 interactive rectangles using @rasenjs/webgl rect() components'
      ]
    })
  ]
})
        ...
        () => {
          switch (testPhase.value) {
            case 'idle':
              return 'Waiting'
            case 'creating':
              return 'Creating 1,000,000 rect() components (WebGL)...'
            case 'ready':
...
const legend = div({
  ...
  children: [
    p({
      children: [
        '📊 WebGL benchmark using GPU-accelerated instanced rendering'
      ]
    }),
    p({
      children: [
        '🎯 Each rectangle is created with @rasenjs/webgl rect() component'
      ]
    }),
    p({
